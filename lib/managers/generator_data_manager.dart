...
class GeneratorDataManager {
  ...
  static Future<void> loadGeneratorData() async {
    ...
    for (int i = 0; i < _generators.length; i++) {
      ...
      print('🔄 GENERATOR_DATA: $name -> fileId: ${fileId.isEmpty ? "EMPTY" : fileId.substring(0, 10)}..., active: $isActive');
    }
  }
  ...
  static Future<void> updateGeneratorFileId(String name, String fileId) async {
    ...
    if (generator != null) {
      ...
      if (index != -1) {
        ...
        print('🔄 GENERATOR_DATA: Updated $name fileId to ${fileId.substring(0, 10)}...');
      }
    }
  }
  ...
  static Future<void> ensureConsistentFileId(String name) async {
    ...
    print('🔍 GENERATOR_DATA: Ensuring consistent fileId for $name on $dateKey');
    
    final currentFileId = await StorageService.getActiveFileId(name);
    if (currentFileId == null || currentFileId.isEmpty) {
      print('⚠️ GENERATOR_DATA: No fileId found for $name, need to create new spreadsheet');
      return;
    }
    ...
    if (generator != null) {
      ...
      if (index != -1) {
        ...
        print('✅ GENERATOR_DATA: Ensured consistent fileId for $name: ${currentFileId.substring(0, 10)}...');
      }
    }
  }
}
